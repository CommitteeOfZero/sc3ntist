...
SCXOffset SCXFile::getReturnAddressOffset(
    const SCXTableIndex returnAddressId) const {
...
void SCXFile::parseHeader() {
  ...
  SCXOffset stringTableOffset =
      *(SCXOffset *)(_data + SCXStringTableOffsetOffset);
  _codeEndOffset = stringTableOffset;
  _stringTable = (SCXOffset *)(_data + stringTableOffset);
  SCXOffset returnAddressTableOffset =
      *(SCXOffset *)(_data + SCXReturnAddressTableOffsetOffset);
  _returnAddressTable = (SCXOffset *)(_data + returnAddressTableOffset);
  _labelTable = (SCXOffset *)(_data + SCXLabelTableOffset);

  _stringCount = (SCXTableIndex)(
      (returnAddressTableOffset - stringTableOffset) / sizeof(SCXOffset));
  if (_stringCount > 0) {
    SCXOffset firstStringOffset = _stringTable[0];
    _returnAddressCount = (SCXTableIndex)(
        (firstStringOffset - returnAddressTableOffset) / sizeof(SCXOffset));
  } else {
    SCXOffset firstReturnAddressOffset = _returnAddressTable[0];
    _returnAddressCount =
        (SCXTableIndex)((firstReturnAddressOffset - returnAddressTableOffset) /
                        sizeof(SCXOffset));
  }
  _labelCount = (SCXTableIndex)((_labelTable[0] - SCXLabelTableOffset) /
                                sizeof(SCXOffset));
}
